...
namespace InventoryClient.Infrastructure.Converters
{
    class RoleidToRolenameConverter: IValueConverter
    {
        ...
        {
            return Task.Run(() => getRoleName((int)value)).Result;
        }
        ...
        private async Task<string> getRoleName(int roleId)
        {
            try
            {
                JsonContent content = JsonContent.Create(roleId);
                ...
                Role clients = JsonSerializer.Deserialize<Role>(responseText)!;
                return clients.Rolename;
            }
